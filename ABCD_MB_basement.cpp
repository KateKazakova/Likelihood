{
  ...
  double HistMaximums[4] = {4000, 1000, 500,1000};
  ...
  for (int i=1; i<=nBins; i++) {
      RooRealVar* bkgA = new RooRealVar(Form("bkgA_bin%d",i), Form("Background in region A, bin %d",i), 1, 0, 1000000);
      RooRealVar* bkgC = new RooRealVar(Form("bkgC_bin%d",i), Form("Background in region C, bin %d",i), 1, 0, 1000000);
      RooRealVar* TFactor = new RooRealVar(Form("TFactor_bin%d",i), Form("Transfer factor (A->B), bin %d",i), 1, 0, 1000000);
  ...
  for(int j=0; j<4; j++)  {
      ...
      {  double N = data_hist -> Integral();
        ...
        TLatex *t3 = new TLatex(-2.2, HistMaximums[j]*0.8, Form("#chi^{2}/N_{dof} = %f", floor(1000000*chi2/(nBins-1))/1000000.0 ));
        t3->Draw();
        cout<<"-----------"<<endl;}
