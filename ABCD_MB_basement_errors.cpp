{
  ...
  int nBins = 4;
  double Left = -3.15, Right = 3.15;
  double MAX = 15000;
  const char* labels[4] = {"A", "B", "C", "D"};
  double HistMaximums[4] = {12000, 1500, 12000, 900};
  ...
  const char* fname_mcbkg = "/home/katet/Programs/MosPhys2022/Files/GammaJetMerge.root";
  const char* fname_signal = "/home/katet/Programs/MosPhys2022/Files/ZgammaMerge.root";
  const char* fname_data = "/home/katet/Programs/MosPhys2022/Files/AsimovData.root";
  const char* fname_znunu = "/home/katet/Programs/MosPhys2022/Files/ZjMerge.root";
  ...
  RooRealVar mu("mu", "mu", 0.5, 0, 1000000);
  ...
  for (int j=0; j<4; j++) {
    for (int i=1; i<=nBins; i++){
        // BRooHist[j] -> SetBinContent(i, ZnunuHist[j]->GetBinContent(i)+100);
        // BRooHist[j] -> SetBinError(i, ZnunuHist[j]->GetBinError(i)+100);
        BRooHist[j] -> SetBinContent(i, 1);
        BRooHist[j] -> SetBinError(i, 0);
  ...
  for (int i=1; i<=nBins; i++) {
      RooRealVar* bkgA = new RooRealVar(Form("bkgA_bin%d",i), Form("Background in region A, bin %d",i), 0.5, 0, 100000);
      RooRealVar* bkgC = new RooRealVar(Form("bkgC_bin%d",i), Form("Background in region C, bin %d",i), 0.5, 0, 100000);
      RooRealVar* TFactor = new RooRealVar(Form("TFactor_bin%d",i), Form("Transfer factor (A->B), bin %d",i), 0.5, 0, 100000);
  ...
  for(int j=0;j<4;j++) {
      ...
      // hist[j] -> GetXaxis() -> SetTitle("ph_{T}, [GeV]");
      //hist[j] -> GetXaxis() -> SetTitle("#eta_{#gamma}");
      hist[j] -> GetXaxis() -> SetTitle("#phi_{#gamma}, [Rad]");
      // hist[j] -> GetXaxis() -> SetTitle("N_{jets}");
  ...
  for(int j=0; j<4; j++)  {
      ...
      {  double N = data_hist -> Integral();
        ...
        double chi2_baker = 0;
        double n_baker, w_baker;
          for (int i=1; i <= nBins; i++) {
          n_baker = data_hist -> GetBinContent(i);
          w_baker = hist[j] -> GetBinContent(i);
          chi2_baker += (n_baker - w_baker + w_baker*(TMath::Log(w_baker/n_baker)));
      }
        cout<<"Chi2 "<<labels[j]<<" = "<<chi2<<endl;
        cout<<"Chi2/ndof "<<labels[j]<<" = "<<chi2/(nBins-1)<<endl;
        cout<<"Chi2 Baker-Cousin"<<labels[j]<<" = "<<chi2_baker<<endl;
        cout<<"Chi2/ndof Baker-Cousin"<<labels[j]<<" = "<<chi2_baker/(nBins-1)<<endl;
        TLatex *t3 = new TLatex(-2.8, HistMaximums[j]*0.8, Form("#chi^{2}/N_{dof} = %f", floor(1000000*chi2/(nBins-1))/1000000.0 ));
        t3->Draw();
        cout<<"-----------"<<endl;}
      ...
      // rat->GetXaxis()->SetTitle("ph_{T}, [GeV]");
      //rat->GetXaxis()->SetTitle("#eta_{#gamma}");
      rat->GetXaxis()->SetTitle("#phi_{#gamma}, [Rad]");
      // rat->GetXaxis()->SetTitleSetTitle("N_{jets}");
    ...
    rat->GetYaxis()->SetTickLength(0.03);
      ...
      for(int i = 1; i<=nBins; i++){
      	rat -> SetBinError(i, DHist[j]->GetBinError(i)/DHist[j]->GetBinContent(i));
  	  }
      if(j==10){
        rat->SetMarkerSize(0);
      for(int i = 1; i<=nBins; i++){
        rat->SetBinContent(i, 1);
        rat->SetBinError(i, 0);
      }}
      rat -> Draw("P E1 SAME");
      if(j!=10){podrelH[j] -> Draw("SAME");
      negrelH[j] -> Draw("SAME");
      rat -> Draw("P E1 SAME");}
